...
# Load data
grade_df <- read_csv("../../Data/grades.csv") %>% mutate(ps_avg = 0.5 * ps1 + 0.5 * ps2)
```

## Summary of grades

- **Min**: `r min(grade_df$midterm, na.rm = T) %>% round(0)`

- **25.super[th]**: `r quantile(grade_df$midterm, probs = 0.25, na.rm = T) %>% round(0)`

- **Mean**: `r mean(grade_df$midterm, na.rm = T) %>% round(0)`

- **Median**: `r median(grade_df$midterm, na.rm = T) %>% round(0)`

- **75.super[th]**: `r quantile(grade_df$midterm, probs = 0.75, na.rm = T) %>% round(0)`

- **Max**: `r max(grade_df$midterm, na.rm = T) %>% round(0)`
---
# Midterm
...
unpause("07_time_series.Rmd", ".", T, T)
```
